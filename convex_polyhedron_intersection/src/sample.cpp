...
void sample1 (){
  ...
  bool solved = convex_polyhedron_intersection::intersection(cube, polygon, out,false);
...
void sample3 (){
  ...
  struct timeval prevTime;
  gettimeofday(&prevTime, NULL);
  bool solved = convex_polyhedron_intersection::intersection(sphere, polygon, out,false);
  struct timeval currentTime;
  gettimeofday(&currentTime, NULL);
  std::cerr << solved << " " << out.rows() << "x"<<out.cols() << " " << (currentTime.tv_sec - prevTime.tv_sec) + (currentTime.tv_usec - prevTime.tv_usec) * 1e-6 << "[s]" <<std::endl;
  std::cerr <<out<<std::endl;;
